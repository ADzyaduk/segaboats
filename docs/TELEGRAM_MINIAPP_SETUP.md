# Настройка Telegram Mini App для Яхты Сочи

## Шаг 1: Создание или настройка бота

### Если бот ещё не создан:

1. Откройте Telegram и найдите @BotFather
2. Отправьте команду `/newbot`
3. Введите имя бота: `Яхты Сочи`
4. Введите username: `YachtsSochiBot` (или другой свободный)
5. Сохраните полученный токен

### Если бот уже есть:

1. Откройте @BotFather
2. Отправьте `/mybots`
3. Выберите вашего бота

## Шаг 2: Настройка Mini App (Web App)

### Вариант A: Через Menu Button (рекомендуется)

1. В @BotFather отправьте `/mybots`
2. Выберите вашего бота
3. Нажмите **Bot Settings**
4. Нажмите **Menu Button**
5. Нажмите **Configure menu button**
6. Введите:
   - **Button text**: `🛥 Каталог яхт`
   - **Web App URL**: `https://v-more.ru`

### Вариант B: Через Web App настройки

1. В @BotFather отправьте `/setmenubutton`
2. Выберите бота
3. Введите URL: `https://v-more.ru`

## Шаг 3: Настройка команд бота

1. В @BotFather отправьте `/setcommands`
2. Выберите бота
3. Отправьте список команд:

```
start - Начать работу с ботом
boats - Каталог яхт
mybookings - Мои бронирования
help - Помощь
```

## Шаг 4: Настройка Webhook

### На сервере выполните:

```bash
# Замените YOUR_BOT_TOKEN на ваш токен
# Замените YOUR_SECRET на случайную строку

curl -X POST "https://api.telegram.org/botYOUR_BOT_TOKEN/setWebhook" \
  -H "Content-Type: application/json" \
  -d '{
    "url": "https://v-more.ru/api/telegram/webhook",
    "secret_token": "YOUR_SECRET",
    "allowed_updates": ["message", "callback_query"]
  }'
```

### Проверка webhook:

```bash
curl "https://api.telegram.org/botYOUR_BOT_TOKEN/getWebhookInfo"
```

Должен вернуть:
```json
{
  "ok": true,
  "result": {
    "url": "https://v-more.ru/api/telegram/webhook",
    "has_custom_certificate": false,
    "pending_update_count": 0
  }
}
```

## Шаг 5: Переменные окружения

Добавьте в `.env` на сервере:

```env
TELEGRAM_BOT_TOKEN=1234567890:ABCdefGHIjklMNOpqrsTUVwxyz
TELEGRAM_BOT_USERNAME=YachtsSochiBot
TELEGRAM_WEBHOOK_SECRET=your_random_secret_string
TELEGRAM_ADMIN_CHAT_ID=123456789
```

### Как узнать TELEGRAM_ADMIN_CHAT_ID:

1. Напишите боту @userinfobot в Telegram
2. Он ответит вашим ID (число)
3. Или создайте группу и добавьте бота @RawDataBot чтобы узнать chat_id группы

## Шаг 6: Перезапуск контейнера

```bash
cd /root/segaboats  # или ваша папка с проектом
docker compose up -d --build app
```

## Шаг 7: Проверка работы

1. Откройте вашего бота в Telegram
2. Нажмите **Start** или отправьте `/start`
3. Должно появиться приветственное сообщение с кнопками
4. Нажмите кнопку меню (слева от поля ввода) - должен открыться Mini App

## Возможные проблемы

### Mini App не открывается

- Проверьте что URL начинается с `https://`
- Проверьте SSL сертификат: `curl -I https://v-more.ru`
- Убедитесь что сайт доступен

### Webhook не работает

- Проверьте логи: `docker logs boats2026-app --tail 50`
- Проверьте что секрет совпадает в `.env` и в `setWebhook`
- Убедитесь что порт 443 открыт

### Бот не отвечает

1. Проверьте токен в `.env`
2. Проверьте webhook: `curl "https://api.telegram.org/botTOKEN/getWebhookInfo"`
3. Если pending_update_count большой, удалите webhook и создайте заново

```bash
# Удалить webhook
curl "https://api.telegram.org/botTOKEN/deleteWebhook"

# Создать заново
curl -X POST "https://api.telegram.org/botTOKEN/setWebhook" ...
```

## Полезные команды BotFather

- `/setname` - изменить имя бота
- `/setdescription` - описание бота (видно при первом открытии)
- `/setabouttext` - текст "О боте"
- `/setuserpic` - фото профиля бота
- `/setcommands` - команды бота
